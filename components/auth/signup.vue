...
const emit = defineEmits(['switch-login-panel'])
...
<template>
  <div class="backdrop mx-auto px-5 fixed w-full h-full inset-0 z-10 " @click.self="emit('close-panel')">
      ...
      <div class="w-full sm:w-2/3 lg:w-1/2 xl:w-2/5 mx-auto border rounded-lg p-5 py-7 bg-white" >
        ...
        <div>
          <ul class="remember-password flex flex-col items-center gap-y-4 sm:flex-row  sm:justify-between md:px-2.5">
            ...
            <li class="text-sm border w-full text-center py-2 rounded sm:rounded-none sm:border-none sm:text-left sm:p-0 sm:w-auto">
              <button class="hover:cursor-pointer" @click="emit('switch-login-panel')">ورود</button>
            </li>
          </ul>
        </div>
      </div>
