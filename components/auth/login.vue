...
const emit = defineEmits(['close-panel', 'switch-login-panel'])
const form = reactive({
  userName: "",
  password: "",
})
...
<template>
  <div class="backdrop mx-auto px-5 w-full fixed inset-0 z-10 h-full " @click.self="emit('close-panel')">
        ...
        <form action="" @submit.prevent="handleSubmit" class=" ">
          ...
          <div class="flex flex-col gap-4 mb-4">
            ...
            <div class="form-group flex flex-col relative gap-3 ">
              ...
              <span class="show-password absolute top-6 left-0 hover:cursor-pointer" @click="showPassword">
        ...
        <div>
          <ul class="remember-password flex flex-col items-center gap-y-4 sm:flex-row  sm:justify-between md:px-2.5">
            ...
            <li class="text-sm border w-full text-center py-2 rounded sm:rounded-none sm:border-none sm:text-left sm:p-0 sm:w-auto">
              <button class="hover:cursor-pointer" @click="emit('switch-login-panel')">ثبت نام</button>
            </li>
          </ul>
        </div>
      </div>
